...
-- ============================================================
-- DAMAGE TIERS
-- Tier 1: 1 fire emitter.  Burst: explosion + sparks
-- Tier 2: 3 fire emitters. Burst: explosion + sparks + electric
-- Tier 3: 6 fire emitters. Burst: big explosion + sparks + electric
--                          + extra Explosion pops on rotor hubs
-- TODO: tune offsets once MI-12 model is available
-- ============================================================
local TIER_OFFSETS = {
    [1] = {
        Vector(  0,   0,  25),
...
local function SpawnBurstFX(ent, count, tier)
    ...
    if tier == 3 then
        -- Rotor hubs instead of wingtips
        for _, side in ipairs({ Vector(140, 0, 20), Vector(-140, 0, 20) }) do
