-- ============================================================
--  MI-12  —  Server-side entity logic
--  Ported from the AN-71 framework (Bombin Addons).
--  All aircraft-specific values are in shared.lua.
-- ============================================================
...
-- TODO: update pass-sounds to MI-12 rotor/turbine assets
local PASS_SOUND_A = "jet/luxor/medium.wav"
local PASS_SOUND_B = "jet/luxor/external.wav"

-- MI-12 model faces +X by default; adjust if mesh differs.
local MODEL_YAW_OFFSET = 180

-- ── Roll / bank tuning ───────────────────────────────────────
-- Helicopters bank more gently than fixed-wing aircraft.
local ROLL_SUSTAINED_GAIN = 1.4
local ROLL_TRANSIENT_GAIN = 35.0
local ROLL_MAX            = 18.0
...
-- ── Orbit wall-probe (identical algorithm to AN-71) ──────────
local PROBE_DIRS = {}
for i = 0, 7 do
...
local function ProbeOrbitRadius(centerPos, skyZ, requestedRadius)
    ...
    if safe < requestedRadius then
        print(string.format("[MI-12] OrbitRadius capped %d -> %d (nearest wall %.0f HU)",
...
function ENT:Initialize()
    ...
    self.Speed        = self:GetVar("Speed",        220)  -- MI-12 is slower than AN-71
    self.OrbitRadius  = self:GetVar("OrbitRadius",  3000)
    self.SkyHeightAdd = self:GetVar("SkyHeightAdd", 5000) -- flies lower than fixed-wing
    self.MaxHP        = self.MaxHP or 10000
    ...
    self.RadialGain     = 0.5
    self.MaxTurnRate    = 22   -- helicopters turn tighter
    self.PrevTurnRate   = 0
    ...
    self.AltDriftRange    = self.AltDriftRange  or 250
    self.AltDriftLerp     = self.AltDriftLerp   or 0.001
    self.JitterPhase      = math.Rand(0, math.pi * 2)
    self.JitterAmplitude  = self.JitterAmplitude or 4
    self.SmoothedRoll     = 0
    ...
    if self.EngineLoop then
        self.EngineLoop:SetSoundLevel(82)
        self.EngineLoop:ChangePitch(85, 0)  -- lower rotor rumble
        self.EngineLoop:ChangeVolume(1.0, 0)
        self.EngineLoop:Play()
    end
    self.PassSoundA = CreateSound(self, PASS_SOUND_A)
    if self.PassSoundA then
        self.PassSoundA:SetSoundLevel(87)
        self.PassSoundA:ChangePitch(85, 0)
    ...
    if self.PassSoundB then
        self.PassSoundB:SetSoundLevel(82)
...
function ENT:OnTakeDamage(dmginfo)
    ...
    local hp = self:GetNWInt("HP", self.MaxHP or 10000)
    hp = hp - dmginfo:GetDamage()
    self:SetNWInt("HP", hp)
    self:Debug("Hit! HP remaining: " .. tostring(hp))
    local tier = CalcTier(hp, self.MaxHP or 10000)
...
function ENT:StartTumble()
    ...
    self.TumbleVelocity = Vector(travelFwd.x * speed, travelFwd.y * speed, -180)
    local sign = function() return (math.random(2) == 1) and 1 or -1 end
    -- Helicopters tumble more violently in yaw/roll when rotors fail
    self.TumbleAngVelocity = Vector(
        math.Rand(60,  180) * sign(),
        math.Rand(10,  60)  * sign(),
        math.Rand(200, 500) * sign()   -- strong autorotation spin
    )
    local pos = self:GetPos()
    local ed  = EffectData()
    ed:SetOrigin(pos) ed:SetScale(4) ed:SetMagnitude(4) ed:SetRadius(400)
    util.Effect("500lb_air", ed, true, true)
    sound.Play("ambient/explosions/explode_4.wav", pos, 135, 85, 1.0)
end
...
-- ── Gib spawner ──────────────────────────────────────────────
-- TODO: replace with MI-12-specific gib models once available.
local GIB_MODELS = {
...
function ENT:CrashExplode()
    ...
    sound.Play("ambient/explosions/explode_8.wav", safePos, 140, 85, 1.0)
    sound.Play("weapon_AWP.Single",                safePos, 145, 58, 1.0)
    util.BlastDamage(self, self, safePos, 450, 220)  -- slightly larger blast radius
...
-- ── Think (same logic as AN-71, identifiers updated) ─────────
function ENT:Think()
...
-- ── PhysicsUpdate (flight + tumble physics) ──────────────────
function ENT:PhysicsUpdate(phys)
    ...
    local targetPitch  = math.Clamp(climbDelta * 5, -7, 7)  -- helicopters pitch less
