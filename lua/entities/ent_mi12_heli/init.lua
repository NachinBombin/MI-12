...
local function ProbeOrbitRadius(centerPos, skyZ, requestedRadius)
    ...
    return math.min(requestedRadius, math.max(200, minDist - PROBE_MARGIN))
end

local function FindGround(centerPos)
    local filterList = {}
    local trace = {
        start  = Vector(centerPos.x, centerPos.y, centerPos.z + 64),
        endpos  = Vector(centerPos.x, centerPos.y, -16384),
        filter = filterList,
    }
    for _ = 1, 100 do
...
local function LaunchManhack(spawnPos, target)
    ...
    toTarget.z = 0
    if toTarget:LengthSqr() > 1 then mh:SetAngles(toTarget:Angle()) end
    mh:Spawn()
    mh:Activate()
    mh:SetEnemy(target)
    if mh.UpdateEnemyMemory then mh:UpdateEnemyMemory(target, target:GetPos()) end
...
function ENT:FadeAndStopSounds(fadeTime)
    ...
    if e then
        e:ChangeVolume(0, t)
        timer.Simple(t + 0.15, function() if e then e:Stop() end end)
    end
end
...
function ENT:Initialize()
    ...
    local radius = ReadParam(self, "OrbitRadius",  3000)
    local skyAdd = ReadParam(self, "SkyHeightAdd", 5000)
    ...
    local spawnPos = Vector(
        center.x + tangent.x * (-radius * math.Rand(0.55, 0.95)),
        center.y + tangent.y * (-radius * math.Rand(0.55, 0.95)),
        skyZ
    )
    if not util.IsInWorld(spawnPos) then spawnPos = Vector(center.x, center.y, skyZ) end
    ...
    self.EngineLoop = CreateSound(self, "mi12_engine_loop")
...
function ENT:Think()
    ...
    if tangentDir:LengthSqr() < 0.001 then
        tangentDir = Angle(0, self.flightYaw, 0):Forward(); tangentDir.z = 0
    end
    ...
    local desired2 = Vector(
        ...
        tangentDir.y + radialDir.y * radialError * self.RadialGain, 0
    )
